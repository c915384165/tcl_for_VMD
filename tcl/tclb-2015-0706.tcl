...
proc resname_log {} {
	...
	set type_list [lsort -unique [[atomselect top all] get type]]
	...
	foreach type_xx $type_list {
		puts $outfile "#$type_xx:"
		echo "$type_xx:"
		# puts $outfile "#[llength [lsort -unique [[atomselect top "resname $resname_xx"] get residue]]]"
		# echo "[llength [lsort -unique [[atomselect top "resname $resname_xx"] get residue]]]"
	}
...
proc load_linshi {number} {
	mol delete all
	cd J:/namd-output/linshi/$number-l/moni
...
# 2 load vmd
	...
	echo 2
# 3 source tcl
	source J:/tcl/tcla/2015-0930/mass-density.tcl
	echo 3
...
# 2 load vmd
	...
	echo 2
# 3 source tcl
	source J:/tcl/tcla/2015-0930/zangle.tcl
	echo 3.1
	source name_num.tcl
	echo 3.2
# 4 run program? no mkdir 
...
# 6 mol delete all
	mol delete all
	echo 6.1
	file copy J:/tcl/tcla/2015-0930/mangle.m .
	echo 6.2
	cd ..
	echo 6.3
...
# 2 load vmd
	...
	echo 2
# 3 source tcl
	source J:/tcl/tcla/2015-0930/scd.tcl
	echo 3.1
	source name_num.tcl
	echo 3.2
# 4 run program? no mkdir 
...
source J:/tcl/tcla/2015-1013-hbond/Hbond.tcl
proc average list {
    expr ([join $list +])/[llength $list]
}
# Hbond_ryan 150 199 "type OG2P1" 4 30 
source J:/tcl/tcla/2015-1012/msd_1010.tcl

source J:/tcl/tcla/2015-1012/source-msd.tcl

proc begin_rdf {} {
	mol delete all
	play step-10.vmd
	source J:/tcl/tcla/2015-1013-rdf/segname.tcl
	# source J:/tcl/tcla/2015-1013-rdf/RDF.tcl
	file mkdir rdf
	cd rdf
	file copy J:/tcl/tcla/2015-1013-rdf/RDF.tcl .
	source RDF.tcl
}

proc get_o_name {} {
	puts "OS3 name [lsort -unique [[atomselect top "type OG2P1"] get name ]]"
	puts "OC2 name [lsort -unique [[atomselect top "type OG2D2"] get name ]]"
	puts "CH3 name [lsort -unique [[atomselect top "type CG331"] get name ]]"
	puts "CF3 name [lsort -unique [[atomselect top "type CG302"] get name ]]"
}